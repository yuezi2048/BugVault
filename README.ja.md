...
### v2.0 新機能 — プロジェクト規約メモリ

- **🏷️ 規約メモリ** — 2 つの新しい MCP ツール（`save_convention` / `retrieve_convention`）で**プロジェクト規約**（アーキテクチャルール、ビジネスルール、テスト規約、スタイルガイド）を保存・検索
- **🧩 共有テーブルアーキテクチャ** — 規約とバグは `bug_records`、`bugvault_chunks` テーブルを共用し、`record_type="convention"` 判別子で区別
- **🔄 同一検索パイプライン** — ベクトル ANN → FTS BM25 → RRF 融合 → Cross-Encoder 再ランク、`record_type` でフィルタリング
- **📦 ConventionRecord モデル** — 4 必須フィールド：`convention_name`、`trigger_context`、`incorrect_behavior`、`correct_behavior`
- **🗄️ 自動スキーマ移行** — `_migrate_v2_schema_if_needed()` が起動時に v1.1.x データベースに `record_type` カラムを追加（冪等、安全）
- **📚 独立した Markdown アーカイブ** — 規約は `~/.bugvault/archive/conventions/` に保存
- 参照: [v2.0 リリースノート](docs/v2.0/v2.0.0-release-notes.md) と [規約仕様](docs/v2.0/convention-spec.md)
...
### 🏷️ `save_convention` — プロジェクト規約メモリ (v2.0)

ユーザーがプロジェクトルール（ビジネスルール、アーキテクチャ規約、テスト規約、スタイルガイド）を修正した場合、AI はこのツールを即座に呼び出してルールを永続化します。

**4 必須フィールド：**

| フィールド | 例 |
|-----------|-----|
| `convention_name` | "API レスポンス形式" |
| `trigger_context` | "REST API エンドポイント作成時" |
| `incorrect_behavior` | "生の dict を返す" |
| `correct_behavior` | "`{"code": 0, "data": ..., "message": "ok"}` 形式を使用" |

**実行フロー：** 同期 Markdown アーカイブ → 非同期埋め込み + LanceDB upsert（`record_type="convention"`）。即座に確認応答。

### 🔍 `retrieve_convention` — プロアクティブ規約チェック (v2.0)

コード変更前に、AI がプロアクティブにこのツールを呼び出し、関連するプロジェクト規約を確認します。

バグ検索と**同一の検索パイプライン**（ベクトル ANN + FTS BM25 → RRF 融合 → Cross-Encoder 再ランク）を使用し、`record_type="convention"` でフィルタリングします。
...
### 🏷️ `ConventionRecord` — プロジェクト規約メモリ (v2.0)

| フィールド | 型 | 必須 | 説明 |
|-----------|-----|:----:|------|
| `convention_name` | `str` (1-256) | ✅ | ルール名 |
| `trigger_context` | `str` (1-32768) | ✅ | 適用タイミング/場所 |
| `incorrect_behavior` | `str` (1-8192) | ✅ | AI がすべきでない動作 |
| `correct_behavior` | `str` (1-16384) | ✅ | AI がすべき動作 |
| `scope` | `str \| None` | ❌ | 適用範囲（例: "src/api/"）|
| `tags` | `str \| None` | ❌ | 分類タグ（例: "Python, FastAPI"）|
| `record_id` | `str \| None` | 🛠️ 自動 | MD5(`convention_name` + `trigger_context`) — 重複排除キー |
| `record_type` | `str` | 🛠️ 自動 | `"convention"` — 判別子 |
| `create_time` | `str` | 🛠️ 自動 | ISO-8601 UTC タイムスタンプ |

BugRecord と**同一のデータベーステーブル**を `record_type="convention"` 判別子で共有。
同じ検索パイプライン（ベクトル ANN + FTS BM25 → RRF 融合 → Cross-Encoder 再ランク）がバグ経験とプロジェクト規約の両方に適用されます。
