...
- **🏷️ 规约记忆** — 新增两个 MCP 工具（`save_convention` / `retrieve_convention`），用于存储和查询**项目规约**——架构规则、业务规则、测试规范、风格指南
- **🧩 共享表架构** — 规约与 bug 共用 `bug_records` 和 `bugvault_chunks` 表，通过 `record_type="convention"` 鉴别器区分
- **🔄 同一检索管线** — 向量 ANN → FTS BM25 → RRF 融合 → Cross-Encoder 精排，现在按 `record_type` 过滤
- **📦 ConventionRecord 模型** — 4 个必填字段：`convention_name`、`trigger_context`、`incorrect_behavior`、`correct_behavior`
- **🗄️ 自动 Schema 迁移** — `_migrate_v2_schema_if_needed()` 在启动时自动为 v1.1.x 数据库添加 `record_type` 列（幂等、无损）
- **📚 独立 Markdown 归档** — 规约归档到 `~/.bugvault/archive/conventions/`
- 参见 [v2.0 发布说明](docs/v2.0/v2.0.0-release-notes.md) 和 [规约规范](docs/v2.0/convention-spec.md)
...
### 🏷️ `save_convention` — 项目规约记忆 (v2.0)

当用户纠正 AI 关于项目规则（业务规则、架构约定、测试规范、风格指南）时，AI 立即调用此工具将规则持久化。
...
| `incorrect_behavior` | "直接返回原始 dict" |
| `correct_behavior` | "使用 `{\"code\": 0, \"data\": ..., \"message\": \"ok\"}` 格式" |

**执行流程：** 同步 Markdown 归档 → 异步嵌入 + LanceDB upsert（`record_type="convention"`）。立即返回确认信息。

### 🔍 `retrieve_convention` — 主动规约检查 (v2.0)

在修改代码前，AI 主动调用此工具检查是否有相关项目规约。

使用与 bug 搜索**相同的检索管线**（向量 ANN + FTS BM25 → RRF 融合 → Cross-Encoder 精排），过滤条件为 `record_type="convention"`。
...
### 🏷️ `ConventionRecord` — 项目规约记忆 (v2.0)

| 字段 | 类型 | 必填 | 说明 |
|------|------|:----:|------|
| `convention_name` | `str` (1-256) | ✅ | 规则名称 |
| `trigger_context` | `str` (1-32768) | ✅ | 适用场景 |
| `incorrect_behavior` | `str` (1-8192) | ✅ | AI 不应做的行为 |
| `correct_behavior` | `str` (1-16384) | ✅ | AI 应做的行为 |
| `scope` | `str \| None` | ❌ | 适用范围（如 "src/api/"） |
| `tags` | `str \| None` | ❌ | 分类标签（如 "Python, FastAPI"） |
| `record_id` | `str \| None` | 🛠️ 自动 | MD5(`convention_name` + `trigger_context`) — 去重键 |
| `record_type` | `str` | 🛠️ 自动 | `"convention"` — 鉴别器 |
| `create_time` | `str` | 🛠️ 自动 | ISO-8601 UTC 时间戳 |

与 BugRecord **共用同一数据库表**，通过 `record_type="convention"` 鉴别器区分。
相同的检索管线（向量 ANN + FTS BM25 → RRF 融合 → Cross-Encoder 精排）同时适用于 bug 经验和项目规约。
