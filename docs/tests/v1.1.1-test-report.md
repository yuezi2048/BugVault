...
对照 `docs/tests/v1.1.1/final-report.md` 中列出的 5 项 P1 问题，逐一核查当前代码库：

| # | 问题 | 根因 | 修复方式 | 验证方法 |
|---|------|------|---------|---------|
| 1 | **LanceDBClient 重复方法定义** | 开发过程中产生的冗余方法 | 清理合并，当前 `lancedb_client.py` 共 15 个方法，无重复定义 | `grep "^    def "` 确认唯一性 |
| 2 | **零向量 parent 回退检索结果随机** | parent 向量优化为全零后，ANN 距离随机 | 回退路径改为纯 FTS 搜索，禁用向量搜索 | `_sync_search_and_format` 第 516 行：`vec_results = []` |
| 3 | **默认 Cross-Encoder 仅英文** | 旧模型 `Xenova/ms-marco-MiniLM-L-6-v2` 仅支持英文 | 切换为 `jinaai/jina-reranker-v2-base-multilingual` | `config.py:42` 验证模型名 |
| 4 | **Embedding 维度切换不兼容** | 换模型后维度变化，旧表无法写入 | 新增 `_verify_vector_dim()`，启动时校验维度，不匹配则报错提示 | `lancedb_client.py:1-19` |
| 5 | **LIKE '%java%' 误中 javascript** | SQL `LIKE` 子串匹配无法区分标签边界 | 新增 `_TECH_EXCLUSIONS` 字典，`LIKE` + `NOT LIKE` 混合过滤 | `tools.py:440-459` |

**结论：5/5 P1 问题已全部闭环，无已知未修复的 P1 问题。**
...
- `tests/test_p0_critical.py` — 483 lines
- `tests/test_p1p2_extended.py` — 570 lines
