...
| 指标 | 数值 |
|------|------|
| Pytest 单元测试 | **69/70** passed (1 deselected e2e) |
| 新增 v1.1.1 管道测试总断言 | **77/77** passed (36 + 41) |
| 测试代码总行数 | 1,643 lines |
| 测试文件数 | 6 files |
| 测试耗时 | 单元测试 17s + 管道测试 6.3s |
| 覆盖特性数 | README 中 25 项特性覆盖 18 项 (72%) |
...
子 Agent 分析发现 README 中 25 项特性有 7 项无测试覆盖：

| 未覆盖特性 | 影响面 | 建议优先级 |
|-----------|--------|-----------|
| LanceDBClient 全部方法搜/存/upsert | 核心数据层 | P0 |
| Cross-Encoder reranker | 精排环节 | P0 |
| `_sync_search_and_format()` 完整管道 | 检索入口 | P0 |
| `_async_embed_and_store()` 异步保存 | 保存入口 | P1 |
| `_compute_suggested_action()` | RAG 评估输出 | P1 |
| `suggest_probe_questions()` | 用户交互 | P2 |
| Stdout 保护层 | MCP 传输安全 | P2 |
...
### 6.1 README 特性 vs 测试覆盖矩阵

| README 特性 | 单元测试 | 集成测试 | 管道测试 | 覆盖状态 |
|-------------|---------|---------|---------|---------|
| BugRecord 模型验证 | ✅ | — | — | ✅ |
...
| ReflectionService | ✅ | — | — | ✅ |
| 父文档映射 | — | — | ✅ | ✅ |
| `_compute_suggested_action` | — | — | — | ❌ |
| Token 统计 | — | — | — | ❌ |
| 双重降级 (quota/exception) | ✅ (quota) | — | — | ⚠️ partial |
| MCP 握手 | — | ✅ | — | ✅ |
| 后向兼容（回退路径） | — | — | — | ❌ |
| rebuild_index.py | — | — | — | ❌ |
| EmbeddingService | — | — | — | ❌ |
...
| 用例 ID | 优先级 | 影响面 | 实现难度 | 推荐测试方式 |
|---------|--------|--------|---------|------------|
| TC-01 | P0 | 检索精度核心环节 | 中 | pytest (mock model) |
| TC-02 | P0 | 检索入口稳定性 | 低 | pytest (mock db) |
| TC-03 | P0 | 升级兼容性 | 低 | pytest (pre-seed old table) |
| TC-04 | P1 | 保存可靠性 | 中 | 集成测试 |
| TC-05 | P1 | RAG 评估输出 | 低 | 纯单元测试 |
| TC-06 | P2 | 过滤精度 | 低 | 单元测试 |
| TC-07 | P2 | 嵌入鲁棒性 | 低 | 单元测试 |
